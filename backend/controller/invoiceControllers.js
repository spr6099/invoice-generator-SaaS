...
// defensive utility function.
function parseItemsField(val) {
  if (!val) return [];
  if (Array.isArray(val)) return val;
  if (typeof val === "string") {
    try {
      return JSON.pare(val);
    } catch {
      return [];
    }
  }
...
export async function getInvoices(req, res) {
  try {
    ...
    const invoices = await Invoice.find(q).sort({ createIndex: -1 }).lean();
...
export async function getInvoiceById(req, res) {
  try {
    ...
    else inv = await findOne({ invoiceNumber: id });
    if (!inv)
      return res
        .status(404)
...
export async function updateInvoice(req, res) {
  try {
    ...
    // So if value is undefined, it deletes that property from object.
